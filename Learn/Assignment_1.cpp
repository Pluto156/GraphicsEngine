...
void printMatrix(const GLfloat* matrix, const char* name) {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            std::cout << matrix[j * 4 + i] << "\t";  // OpenGL 以列优先存储矩阵
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
...
void display() {
    ...
    static GLfloat lastModelViewMatrix[16] = { 0 };  // 存储上次的矩阵，初始为 0
    ...
    if (memcmp(lastModelViewMatrix, modelViewMatrix, sizeof(modelViewMatrix)) != 0) {
        ...
        memcpy(lastModelViewMatrix, modelViewMatrix, sizeof(modelViewMatrix));  // 更新缓存
    }
...
void drawStage()
{
    if (!isInitStage)
    {
        ...
        // c区：4块3x11x0.5的舞台块，左右各2块，间距0.05
        AreaPos = C.position;
...
void processKeyboard(unsigned char key, int x, int y)
{
    ...
    camera.processKeyboard(key, x, y);  // 使用 Camera 对象来处理键盘输入
    glutPostRedisplay();
}
...
void processMouse(int button, int state, int x, int y)
{
    ...
    camera.processMouse(button, state, x, y);  // 使用 Camera 对象来处理鼠标输入
}

// 处理鼠标移动
void processMouseMotion(int x, int y)
{
    camera.processMouseMotion(x, y);  // 使用 Camera 对象来处理鼠标移动
    //camera.CameraDebug();
}
