...
namespace PhysicsLit
{
	...
	{
		// 线性速度改变量
		CVector3 linearVelocityChange[2];
		// 角速度改变量
		CVector3 angularVelocityChange[2];
		// 最大速率
		float maxSpeed;
					...
					if (contacts[i].mRigidBodies[j])
					{
						for (uint32_t k = 0; k < 2; k++)
						{
							if (contacts[i].mRigidBodies[j] == contacts[idx].mRigidBodies[k])
							{
								...
								contacts[i].mContactVelocity += contacts[i].mContactToWorld.GetTranspose() * deltaVelocity * (j ? -1.0f : 1.0f);
								// 更新期望速度变化量
