...
{
	Contact::Contact(RigidBodyPrimitive* rigidBody1, RigidBodyPrimitive* rigidBody2)
	{
	...
	void Contact::SetRigidBodies(RigidBodyPrimitive* rigidBody1, RigidBodyPrimitive* rigidBody2)
	{
		mRigidBodies[0] = rigidBody1;
		mRigidBodies[1] = rigidBody2;
		UpdateCoefficient();
	...
	void Contact::SwapRigidBodies()
	{
		...
		RigidBodyPrimitive* temp = mRigidBodies[0];
	...
	void Contact::ResolvePenetration(CVector3 linearChange[2], CVector3 angularChange[2], float penetration)
	{
		// 两个刚体需要通过线性运动来移动的距离
		...
		for (uint32_t i = 0; i < 2; i++)
		{
			if (mRigidBodies[i])
			{
				...
				CVector3 velocityPerUnitImpulseWorld = Math::Cross(mRelativeContactPosition[i],mContactNormal);
		...
		for (uint32_t i = 0; i < 2; i++)
		{
			if (mRigidBodies[i])
			{
				...
				float limit = 0.2f * mRelativeContactPosition[i].len();
				...
				else
				{
					...
					CVector3 angularImpulsePerUnitLinearImpulse = Math::Cross(mRelativeContactPosition[i],mContactNormal);
					// 同上，单位线性冲量带来的角速度变化
					CVector3 angularVelocityPerLinearImpulse = mRigidBodies[i]->GetInverseInertiaTensorWorld() * angularImpulsePerUnitLinearImpulse;
					...
					CVector3 angularPerMove = angularVelocityPerLinearImpulse / angularInertia[i];
	...
	void Contact::ResolveVelocityChange(CVector3 linearVelocityChange[2], CVector3 angularVelocityChange[2])
	{
		// 获取两个刚体的逆惯性张量
		CMatrix3 inverseInertiaTensor[2];
		...
		// 转换到世界空间
		CVector3 impulseWorld = mContactToWorld * impulseContact;

		// 计算角冲量，Jt = r x Jf
		CVector3 angularImpulse = Math::Cross(mRelativeContactPosition[0],impulseWorld);
		...
		{
			// 对于第二个刚体，冲量的作用是相反的
			CVector3 i2 = Math::Cross(impulseWorld,mRelativeContactPosition[1]);
	...
	CVector3 Contact::CalculateFrictionImpulse(CMatrix3* inverseInertiaTensor)
	{
		// 冲量公式为: J = m * v
		...
		CMatrix3 impulseToTorque(mRelativeContactPosition[0]);
		...
		CMatrix3 deltaVelocityWorld = impulseToTorque * inverseInertiaTensor[0] * impulseToTorque * -1.0f;
		...
		if (mRigidBodies[1])
		{
			impulseToTorque = CMatrix3(mRelativeContactPosition[1]);

			CMatrix3 deltaVelocityWorld2 = impulseToTorque * inverseInertiaTensor[1] * impulseToTorque * -1.0f;
		...
		// 把速度变化矩阵转换到碰撞空间
		CMatrix3 deltaVelocityContact = mContactToWorld.GetTranspose() * deltaVelocityWorld * mContactToWorld;
		...
		deltaVelocityContact += CMatrix3(linearVelocity);
		...
		// 以此类推......
		CMatrix3 impulsePerUnitVelocity = Math::Inverse(deltaVelocityContact);

		// 这是两个对象碰撞时要变化的速度(碰撞空间)
		// x方向是碰撞法线方向，变化速度就是本次碰撞期望产生的，在碰撞法线上的变化速度
		// y和z是会产生摩擦力的平面，这两个方向上的速度会因为摩擦力抵消掉(这一次碰撞可能不会完全抵消完)
		CVector3 velocityToRemove(mDesiredDeltaVelocity, -mContactVelocity.y, -mContactVelocity.z);
		...
		CVector3 impulseContact = impulsePerUnitVelocity * velocityToRemove;
		...
		if (planarImpulse > impulseContact.x * mFriction)
		{
			...
			CVector3 velocityOnXPerUnitImpulse = deltaVelocityContact.GetRow(0);
	...
	CVector3 Contact::CalculateFrictionlessImpulse(CMatrix3* inverseInertiaTensor)
	{
		// 单位线性冲量带来的旋转导致的速度变化(解释见ResolvePenetration)
		CVector3 velocityPerUnitImpulseByRotation = Math::Cross(mRelativeContactPosition[0],mContactNormal);
		...
		{
			// 过程同上
			CVector3 v2 = Math::Cross(mRelativeContactPosition[1],mContactNormal);
		...
		return CVector3(mDesiredDeltaVelocity / velocityPerUnitImpulse, 0.0f, 0.0f);
	}
	...
	void Contact::UpdateOrthogonalBasis()
	{
		...
		// 构造坐标系旋转矩阵
		mContactToWorld = CMatrix3(mContactNormal.x, axisY.x, axisZ.x,
	...
	CVector3 Contact::CalculateLocalVelocity(uint32_t index, float duration)
	{
		auto rigidBody = mRigidBodies[index];

		// 先计算碰撞点由于旋转产生的速度
		CVector3 velocity = Math::Cross(rigidBody->GetAngularVelocity(),mRelativeContactPosition[index]);
		// 再加上刚体的线性速度得到碰撞点的当前速度(世界坐标系)
		velocity += rigidBody->GetVelocity();

		// 将速度转换到碰撞坐标系(非碰撞法线上的速度分量体现了对象间彼此的移动速度，计算摩擦力的时候需要用)
		CVector3 contactVelocity = mContactToWorld.GetTranspose() * velocity;

		// 在不考虑反作用力的情况下，加速度带来的速度变化
		CVector3 deltaVelocity = rigidBody->GetLastAcceleration() * duration;
		// 转换到碰撞坐标系
		deltaVelocity = mContactToWorld.GetTranspose() * deltaVelocity;
