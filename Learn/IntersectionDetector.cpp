...
namespace PhysicsLit
{
	...
	{
		// 将射线转换到Box的局部空间
		CMatrix4 transform = box.transformMatrix.GetInverse();
	...
	bool IntersectionDetector::Detect(const Ray& localRay, const CVector3& boxHalfSize, RayHitInfo& hit)
	{
		// 射线距离的最小最大值
		float tMin = 0.0f;
		float tMax = FLT_MAX;
		...
		// 射线方向的倒数
		CVector3 inverseDirection = CVector3(1.0f / localRay.mDirection.x, 1.0f / localRay.mDirection.y, 1.0f / localRay.mDirection.z);

		// Box的最小最大值
		CVector3 bMin = -boxHalfSize;
		CVector3 bMax = boxHalfSize;
	...
	{
		CMatrix4 iTrans = plane.transformMatrix.GetInverse();
	...
	{
		CVector3 m = ray.mOrigin - sphere.transformMatrix.GetColumn(3);
	...
	{
		CMatrix4 iTrans = circle.transformMatrix.GetInverse();
		// 变化射线的方向向量不同于变化法线，直接用和变化顶点相同的矩阵即可
		Ray localRay = Ray(iTrans * ray.mOrigin.ToPosVec4(), iTrans * ray.mDirection.ToDirVec4());
		...
		CVector3 pHit = localRay.mOrigin + hit.distance * localRay.mDirection;

		return pHit.lenSquared() <= circle.mRadius * circle.mRadius;
	}

	// Moller-Trumbore intersection algorithm
	// "Fast, Minimum Storage Ray-Triangle Intersection", Journal of Graphics Tools, vol. 2, no. 1, pp 21-28, 1997.
	bool IntersectionDetector::Detect(const Ray& ray, const CVector3& v0, const CVector3& v1, const CVector3& v2, float& t)
	{
		CVector3 e1 = v1 - v0;
		CVector3 e2 = v2 - v0;
		CVector3 p = Math::Cross(ray.mDirection,e2);
	...
	{
		CVector3 centerLine = box1.transformMatrix.GetColumn(3) - box2.transformMatrix.GetColumn(3);

		// 分离轴定理(若两个凸面体不相交，则一定至少存在一根轴，使这两个凸面体投影到这根轴上后没有重叠)
		return
			IsOverlapOnAxis(box1, box2, box1.transformMatrix.GetColumn(0), centerLine) &&
			IsOverlapOnAxis(box1, box2, box1.transformMatrix.GetColumn(1), centerLine) &&
			IsOverlapOnAxis(box1, box2, box1.transformMatrix.GetColumn(2), centerLine) &&

			IsOverlapOnAxis(box1, box2, box2.transformMatrix.GetColumn(0), centerLine) &&
			IsOverlapOnAxis(box1, box2, box2.transformMatrix.GetColumn(1), centerLine) &&
			IsOverlapOnAxis(box1, box2, box2.transformMatrix.GetColumn(2), centerLine) &&

			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(0), box2.transformMatrix.GetColumn(0)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(0), box2.transformMatrix.GetColumn(1)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(0), box2.transformMatrix.GetColumn(2)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(1), box2.transformMatrix.GetColumn(0)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(1), box2.transformMatrix.GetColumn(1)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(1), box2.transformMatrix.GetColumn(2)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(2), box2.transformMatrix.GetColumn(0)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(2), box2.transformMatrix.GetColumn(1)), centerLine) &&
			IsOverlapOnAxis(box1, box2, Math::Cross(box1.transformMatrix.GetColumn(2), box2.transformMatrix.GetColumn(2)), centerLine);
	}

	bool IntersectionDetector::Detect(const CollisionSphere& sphere1, const CollisionSphere& sphere2)
	{
		CVector3 centerLine = sphere1.transformMatrix.GetColumn(3) - sphere2.transformMatrix.GetColumn(3);
		return centerLine.lenSquared() < (sphere1.mRadius + sphere2.mRadius) * (sphere1.mRadius + sphere2.mRadius);
	}
	...
	bool IntersectionDetector::DetectBoxAndHalfSpace(const CollisionBox& box, const CollisionPlane& plane)
	{
		...
		// Box到平面的距离
		float distance = Math::Dot(plane.mNormal, CVector3(box.transformMatrix.GetColumn(3))) - projectedLength;
	...
	{
		float distance = Math::Dot(plane.mNormal, CVector3(sphere.transformMatrix.GetColumn(3))) - sphere.mRadius;
		return distance <= plane.mDistance;
	}

	bool IntersectionDetector::IsOverlapOnAxis(const CollisionBox& box1, const CollisionBox& box2, const CVector3& axis, const CVector3& centerLine)
	{
	...
	float IntersectionDetector::GetPenetrationOnAxis(const CollisionBox& box1, const CollisionBox& box2, const CVector3& axis, const CVector3& centerLine)
	{
		float projectedLength1 = box1.GetHalfProjectedLength(axis);
		float projectedLength2 = box2.GetHalfProjectedLength(axis);
	...
	bool IntersectionDetector::DetectLineSegmentContact(
		const CVector3& midPoint1, const CVector3& dir1, float halfLength1,
		const CVector3& midPoint2, const CVector3& dir2, float halfLength2,
		CVector3& contactPoint, bool useOne)
	{
		// 两条线段方向长度的平方
		float squaredLength1 = dir1.lenSquared();
		float squaredLength2 = dir2.lenSquared();
		// 两条线段方向的点积
		...
		// 点2到点1的向量
		CVector3 p2top1 = midPoint1 - midPoint2;
		// 点2到点1的向量和两条线段方向的点积
		...
		{
			// 通过线段1计算交点
			CVector3 contactPos1 = midPoint1 + dir1 * distance1;
			// 通过线段2计算交点
			CVector3 contactPos2 = midPoint2 + dir2 * distance2;
