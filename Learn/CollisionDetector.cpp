...
namespace PhysicsLit
{
	...
	{
		// 碰撞法线为Box1对应面的法线
		CVector3 normal = box1.transformMatrix.GetColumn(axisIdx);
		// 这里是让碰撞法线方向始终是从Box2到Box1的
		if (Math::Dot(normal, centerLine) > 0.0f)
			normal = normal * -1.0f;

		// 计算具体是Box2的哪个顶点发生了碰撞
		CVector3 vertex = box2.mHalfSize;
		// 如果Box2在Box1的左侧，那么Box2的顶点的X坐标为Box2的半长
		if (Math::Dot(CVector3(box2.transformMatrix.GetColumn(0)), normal) < 0.0f)
			vertex.x = -vertex.x;
		// 如果Box2在Box1的下侧，那么Box2的顶点的Y坐标为Box2的半宽
		if (Math::Dot(CVector3(box2.transformMatrix.GetColumn(1)), normal) < 0.0f)
			vertex.y = -vertex.y;
		// 如果Box2在Box1的后侧，那么Box2的顶点的Z坐标为Box2的半高
		if (Math::Dot(CVector3(box2.transformMatrix.GetColumn(2)), normal) < 0.0f)
			vertex.z = -vertex.z;
		...
		// 碰撞点(Box2顶点变换到世界坐标系)
		contact->mContactPoint = box2.transformMatrix * vertex;
		// 碰撞深度
		contact->mPenetration = penetration;
		contact->SetRigidBodies(box1.rigidBodyPrimitive, box2.rigidBodyPrimitive);
	}
...
#define CHECK_OVERLAP(axis, index) if (!TryAxis(box1, box2, (axis), centerLine, (index), penetration, axisIdx)) return 0;
	...
	{
		CVector3 centerLine = box2.transformMatrix.GetColumn(3) - box1.transformMatrix.GetColumn(3);
		...
		// 分别检测两个Box的三条轴
		CHECK_OVERLAP(box1.transformMatrix.GetColumn(0), 0);
		CHECK_OVERLAP(box1.transformMatrix.GetColumn(1), 1);
		CHECK_OVERLAP(box1.transformMatrix.GetColumn(2), 2);

		CHECK_OVERLAP(box2.transformMatrix.GetColumn(0), 3);
		CHECK_OVERLAP(box2.transformMatrix.GetColumn(1), 4);
		CHECK_OVERLAP(box2.transformMatrix.GetColumn(2), 5);

		// 计算两个Box的三条轴的叉乘轴
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(0), box2.transformMatrix.GetColumn(0)), 6);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(0), box2.transformMatrix.GetColumn(1)), 7);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(0), box2.transformMatrix.GetColumn(2)), 8);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(1), box2.transformMatrix.GetColumn(0)), 9);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(1), box2.transformMatrix.GetColumn(1)), 10);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(1), box2.transformMatrix.GetColumn(2)), 11);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(2), box2.transformMatrix.GetColumn(0)), 12);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(2), box2.transformMatrix.GetColumn(1)), 13);
		CHECK_OVERLAP(Math::Cross(box1.transformMatrix.GetColumn(2), box2.transformMatrix.GetColumn(2)), 14);
		...
		else
		{
			...
			CVector3 axis1 = box1.transformMatrix.GetColumn(box1AxisIdx);
			CVector3 axis2 = box2.transformMatrix.GetColumn(box2AxisIdx);
			CVector3 axis = Math::Cross(axis1, axis2);
			axis.Normalize();
			...
			CVector3 midPoint1 = box1.mHalfSize;
			CVector3 midPoint2 = box2.mHalfSize;
			for (uint32_t i = 0; i < 3; ++i)
			{
				if (i == box1AxisIdx)
					midPoint1[i] = 0.0f;
				else if (Math::Dot(CVector3(box1.transformMatrix.GetColumn(i)), axis) > 0.0f)
					midPoint1[i] = -midPoint1[i];

				if (i == box2AxisIdx)
					midPoint2[i] = 0.0f;
				else if (Math::Dot(CVector3(box2.transformMatrix.GetColumn(i)), axis) < 0.0f)
					midPoint2[i] = -midPoint2[i];
			}

			// 把这两个中点转换到世界坐标系下
			midPoint1 = box1.transformMatrix * CVector4(midPoint1, 1.0f);
			midPoint2 = box2.transformMatrix * CVector4(midPoint2, 1.0f);
	...
	uint32_t CollisionDetector::Detect(const CollisionBox& box, const CollisionSphere& sphere, CollisionData* data)
	{
		...
		// 世界坐标系下的球心
		CVector3 gSphereCenter = sphere.transformMatrix.GetColumn(3);
		// Box坐标系下的球心
		CVector3 lSphereCenter = box.transformMatrix.GetInverse() * CVector4(gSphereCenter, 1.0f);
		...
		// Box上离球心最近的点
		CVector3 closestPoint(0.0f, 0.0f, 0.0f);
		// 临时变量
		float distance;
		...
		// 判断是否相交
		distance = (closestPoint - lSphereCenter).lenSquared();
		if (distance > sphere.mRadius * sphere.mRadius)
			return 0;

		// 变换到世界坐标系
		closestPoint = box.transformMatrix * CVector4(closestPoint, 1.0f);

		// 当前要写入的碰撞
		Contact* contact = data->mCurContact;
		// 碰撞法线
		contact->mContactNormal = (closestPoint - gSphereCenter).GetNormalize();
		// 碰撞点
		contact->mContactPoint = closestPoint;
		// 碰撞深度
		contact->mPenetration = sphere.mRadius - sqrtf(distance);
		contact->SetRigidBodies(box.rigidBodyPrimitive, sphere.rigidBodyPrimitive);
	...
	uint32_t CollisionDetector::Detect(const CollisionSphere& sphere1, const CollisionSphere& sphere2, CollisionData* data)
	{
		...
		// 两个碰撞球的位置
		CVector3 pos1 = sphere1.transformMatrix.GetColumn(3);
		CVector3 pos2 = sphere2.transformMatrix.GetColumn(3);

		CVector3 centerLine = pos1 - pos2;
		...
		Contact* contact = data->mCurContact;
		// 碰撞法线
		contact->mContactNormal = centerLine.GetNormalize();
		// 碰撞点(第一个球的球面和球心连线交点)
		...
		contact->SetRigidBodies(sphere1.rigidBodyPrimitive, sphere2.rigidBodyPrimitive);

		data->AddContacts(1);
		return 1;
	}

	static const CVector3 boxVertex[8] =
	{
	...
	uint32_t CollisionDetector::Detect(const CollisionBox& box, const CollisionPlane& plane, CollisionData* data, bool isHalfSpace)
	{
		...
		{
			// 世界坐标系下的Box顶点
			CVector3 vertexPos = box.transformMatrix * CVector4(boxVertex[i] * box.mHalfSize, 1.0f);
			...
			if (distance <= plane.mDistance)
			{
				...
				contact->SetRigidBodies(box.rigidBodyPrimitive, nullptr);
	...
	uint32_t CollisionDetector::DetectSphereAndPlane(const CollisionSphere& sphere, const CollisionPlane& plane, CollisionData* data)
	{
		...
		// 碰撞球的位置
		CVector3 pos = sphere.transformMatrix.GetColumn(3);
		...
		// 碰撞深度(碰撞点到平面的距离)
		contact->mPenetration = penetration;
		contact->SetRigidBodies(sphere.rigidBodyPrimitive, nullptr);
